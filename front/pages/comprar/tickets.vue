<template>
    <div class="min-h-screen flex justify-center items-center p-4">
        <div
            ...
            <div v-if="movieData?.title" class="p-6 border-gray-700 flex items-center"
                :style="{ backgroundImage: `url(${movieData.poster})`, backgroundPosition: 'center', backgroundSize: 'cover' }">
                <div class="flex items-center bg-black/70 w-full border border-gray-600 p-3 rounded-lg">
                    <div
                        class="h-12 w-12 flex-shrink-0 bg-gray-700 border border-gray-600 rounded-full flex items-center justify-center text-white">
            ...
            <div class="p-6 bg-gray-800/30 backdrop-blur-md">
                <form @submit.prevent="ticketForm" class="space-y-5">
                    <div class="space-y-2" v-if="!authStore.isAuthenticated">
                        ...
                        <div class="relative">
                            ...
                            <input id="nombre" v-model="name" placeholder="Ingresa tu nombre"
                    ...
                    <div class="space-y-2" v-if="!authStore.isAuthenticated">
                        ...
                        <div class="relative">
                            ...
                            <input id="correo" v-model="email" type="email" placeholder="Ingresa tu correo"
                                class="pl-10 border border-gray-600 p-3 rounded-lg w-full focus:outline-none focus:ring-2 focus:ring-gray-400 bg-gray-800/60   text-white"
                                required />
                        </div>
                    </div>
            ...
            <div
                class="p-4 bg-gray-900/80 text-gray-300 text-center text-sm border-t border-gray-700 flex items-center justify-center">
...
const name = ref(authStore.isAuthenticated ? authStore.user.name : '');
const email = ref(authStore.isAuthenticated ? authStore.user.email : '');

async function ticketForm() {
    const loadingElement = document.createElement('div');
    loadingElement.className = 'fixed inset-0 flex items-center justify-center bg-black bg-opacity-70 z-50';
    loadingElement.innerHTML = `
        <div class="bg-gray-800/90 backdrop-blur-md p-6 rounded-lg shadow-xl border border-gray-700">
            ...
            <h3 class="text-xl font-bold text-center text-gray-100">Procesando compra...</h3>
            <p class="text-gray-300 text-center mt-2">Por favor, espera un momento</p>
        </div>
    `;
    ...
    try {
        ...
        errorElement.className = 'fixed inset-0 flex items-center justify-center bg-black bg-opacity-70 z-50';
        errorElement.innerHTML = `
            <div class="bg-gray-800/90 backdrop-blur-md p-6 rounded-lg shadow-xl border border-gray-700">
                <div class="text-red-400 flex justify-center mb-4">
                    <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-10">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v3.75m9-.75a9 9 0 1 1-18 0 9 9 0 0 1 18 0Zm-9 3.75h.008v.008H12v-.008Z" />
                    </svg>
                </div>
                <h3 class="text-xl font-bold text-center text-gray-100">Error al procesar la compra</h3>
                <p class="text-gray-300 text-center mt-2">Por favor, inténtalo de nuevo más tarde</p>
            </div>
        `;
