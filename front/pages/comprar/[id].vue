    ...
    <div class="relative rounded-3xl overflow-hidden mb-8 shadow-2xl">
      <div class="relative h-64 md:h-96 w-full">
        <div 
          ...
          :style="{ backgroundImage: `url(${movieData.backdrop || movieData.poster})` }"
        ></div>
      ...
      <div class="relative z-10 -mt-32 px-6 pb-6 flex flex-col md:flex-row items-start md:items-end gap-6">
        <!-- Poster -->
        <div class="w-32 md:w-48 flex-shrink-0 rounded-xl overflow-hidden shadow-lg border-4 border-gray-800 transform translate-y-0 md:-translate-y-12">
        ...
        <div class="flex-1">
          ...
          <div class="flex flex-wrap gap-3 mb-3">
            ...
            <p v-if="movieData.duration" class="bg-gray-800 px-3 py-1 rounded-full text-sm">{{ movieData.duration }} mins</p>
            <p v-if="movieData.rating" class="bg-yellow-600 px-3 py-1 rounded-full text-sm flex items-center gap-1">
              <p>★</p> {{ movieData.rating }}
            </p>
          </div>
      ...
      <div class="bg-gray-800/50 backdrop-blur-sm rounded-2xl p-6 shadow-xl border border-gray-700">
        ...
        <div class="space-y-4">
          ...
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 mt-6">
            ...
            >
              <div 
                class="bg-gray-700 px-4 py-4 rounded-xl flex flex-col items-center transition-all duration-200"
                :class="selectedTime === hora ? 'bg-blue-600' : 'hover:bg-gray-600'"
              >
                <span class="text-lg font-bold">{{ hora }}</span>
              </div>
            </button>
          </div>
...
const dias = ["Hoy", "Mañana", "Miércoles", "Jueves"];
const horarios = ["16:00", "18:30", "21:00", "23:15"];
